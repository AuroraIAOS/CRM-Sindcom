...
- **Dados de demonstração: a regra MUDOU em 2026-09-09, com o início da Onda 01.** Até ali, dado DEMO ficava gravado para dar a Maxwell a visão incremental do sistema funcionando. **Esse período acabou:** a base agora recebe cadastro real de 9.186 caixas, e dado fictício convivendo com ele deixou de ser demonstração e virou poluição — inflando cobertura, remessas e contagens que dirigem decisão de campanha.

  **A base foi limpa em 2026-09-09** (55 trabalhadores, 3 campanhas DEMO, 10 envios, 13 remessas, 3 descadastros, 3 contabilidades e 10 empresas fictícias). Sobrou **um** trabalhador: `Isac Henrique Machado Rufino`, real, vindo do formulário do site, ainda pendente de aprovação — **não apagar, não aprovar sem ordem do Maxwell**.

  **Daqui em diante:** dado de verificação criado numa subetapa é **removido ao final dela**, no mesmo turno em que foi criado, e o relatório diz o que foi criado e o que foi removido. Se algum caso exigir que fique, isso é decisão do Maxwell, pedida explicitamente — não presumida. O prefixo `DEMO —` continua obrigatório enquanto o registro existir, e é ele que torna a limpeza possível por critério de **prefixo** (`nome like 'DEMO%'`), nunca por "contém": filtrar por conteúdo pegou `contabilida​demo​ntanari` e `diegomara​demo​rais`, que são reais.
