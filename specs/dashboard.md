...
> ¹ O K1 do Jurídico usa **consulta direta** a `trabalhadores` (count por nível) — não a `v_dash_kpis`. **Corrigido em 2026-07-21 (Subetapa 03.1), medido com login real:** a razão registrada aqui antes — "o RLS nega as subqueries financeiras e derruba a view inteira" — está errada. A view **não** falha para o Jurídico: ela responde 200 com uma linha em que `guias_em_atraso` e `boletos_inadimplentes` valem **0**, porque a RLS filtra `faturas`/`repasses` (ele enxerga 0 linhas nas duas tabelas). Pior ainda, o MRR que ele vê é o valor **real**, já que `v_mensalidade_titular`/`v_base_calculo_trabalhador` são cadastrais e estão no acesso dele. Ou seja: renderizar o K4 para o Jurídico não daria erro — daria "nenhuma inadimplência" como se fosse fato. É o modo de falha "200 + zero itens" (`orientacoes.md` §3.2/§7.2). Fixado por teste em `tests/rls/dashboard.spec.ts`. Detalhe análogo e pela mesma mecânica: `fila_admin_pendente` conta sob o RLS de quem consulta (Presidente leria 0 — por isso o K5 não renderiza para ele).
...
O corte por role é dupla camada, mas as duas camadas fazem coisas **diferentes** — e confundi-las é perigoso. As views `security_invoker = on` garantem que ninguém **extraia dado alheio**: a RLS filtra as tabelas de base, e quem não tem acesso recebe zero. O que a RLS *não* faz é recusar a consulta — o zero volta com status 200 e cara de resposta legítima. Por isso o recorte de **tela** não é redundância cosmética: é ele que impede o painel de afirmar "R$ 0,00 de inadimplência" para quem, na verdade, apenas não pode ver a inadimplência. Regra prática: **se um widget depende de tabela fora do acesso do papel, não renderize o widget — nunca confie na view para "dar erro".**
