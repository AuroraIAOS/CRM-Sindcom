...
import { clienteAnon, ehErroRls, loginComo } from "./helpers";
...
describe("03.1 · corte por papel (a RLS zera, não recusa)", () => {
  it("jurídico lê v_dash_kpis com o financeiro ZERADO — por isso a tela dele é outra", async () => {
    ...
    const juridico = await loginComo("juridico");

    const { data: comoAdmin } = await admin.client.from("v_dash_kpis").select("*").single();
    const { data: comoJuridico, error } = await juridico.client
      .from("v_dash_kpis")
      .select("*")
      .single();
    ...
    // renderizar o K4.
    const { count: repassesVisiveis } = await juridico.client
      .from("repasses")
      .select("id", { count: "exact", head: true });
    const { count: faturasVisiveis } = await juridico.client
      .from("faturas")
      .select("id", { count: "exact", head: true });
    ...
    await juridico.client.auth.signOut();
  }, 120_000);

  it("jurídico não alcança as views financeiras de série", async () => {
    const { client } = await loginComo("juridico");
  ...
  it("parceiro não enxerga nada da base pelo dashboard", async () => {
    ...
    const { data } = await client.from("v_dash_kpis").select("*").single();
    ...
    const { data: dicas } = await client.from("v_dash_dicas").select("*");
  ...
  it("presidente e secretária leem os indicadores de gestão", async () => {
    for (const papel of ["presidente", "secretaria"] as const) {
      ...
      const { data, error } = await client.from("v_dash_kpis").select("*").single();
      expect(error, `${papel} deveria ler v_dash_kpis`).toBeNull();
      expect(data!.total_trabalhadores).toBeGreaterThan(0);

      const { error: erroMapa } = await client.from("v_dash_mapa").select("*").limit(1);
...
describe("03.1 · snapshot mensal (G1)", () => {
  it("só o Admin tira a fotografia; a secretária é barrada pela guarda", async () => {
    const secretaria = await loginComo("secretaria");
    const { error } = await secretaria.client.rpc("fn_snapshot_dashboard");
    ...
    await secretaria.client.auth.signOut();

    const admin = await loginComo("admin");
    const { error: erroAdmin } = await admin.client.rpc("fn_snapshot_dashboard");
    ...
    const { data: fotos } = await admin.client
    ...
    const { error: erroRepetido } = await admin.client.rpc("fn_snapshot_dashboard");
